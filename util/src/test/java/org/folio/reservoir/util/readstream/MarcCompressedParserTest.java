...
public class MarcCompressedParserTest {
  Vertx vertx;
  @Before
  public void before() {
    vertx = Vertx.vertx();
  ...
  Future<MappingReadStream<Record, Buffer>> marc4jParserFromFile(String fname) {
    return vertx.fileSystem().open(fname, new OpenOptions())
        .map(file -> new MappingReadStream<>(new MappingReadStream<>(file, new GzipDecompressMapper()), new Marc4jMapper()));
  }

  Future<MappingReadStream<Record, Buffer>> marc4jParserFromFile() {
    return marc4jParserFromFile("marc3.marc.gz");
  }
